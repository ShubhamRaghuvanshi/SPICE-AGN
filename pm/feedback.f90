     ...
     do j=1,nSNnei
        ...
        if(cpu_map(father(igrid)).eq.myid) then ! if belong to myid
           ...
           if(mechanical_geen)then 
            ...
            chi_tr   = psn_tr**2d0 / (2d0 * num_sn*(1d51/msun2g/km2cm**2d0) * M_SN_var * f_ESN) 
           ...
           !f_w_crit = max(0d0,f_w_crit)
            vload_rad = dsqrt(2d0*f_ESN*nSN(i)*E_SNII*(1d0+f_w_crit)/(M_SN_var*msun2g))/scale_v/(1d0+f_w_cell)/f_LOAD/f_PCAN
           ...
           if(f_w_cell.ge.f_w_crit)then ! radiative phase
              ...
              f_esn2 = 1d0-(1d0-f_ESN)*f_w_cell/f_w_crit ! to smoothly correct the adibatic to the radiative phase
               vload = dsqrt(2d0*f_esn2*nSN(i)*E_SNII/(1d0+f_w_cell)/(M_SN_var*msun2g))/scale_v/f_LOAD
...
subroutine mech_fine_mpi(ilevel)
  ...
  real(dp)::num_sn,vload,Zdepen=1d0,vol_nei,dm_ejecta, nSN_i
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  ...
  do i=1,np
     ...
     nSN_i     = SNrecv(11,i) ! doesn't have to be an integer
     ...
     do j=1,nSNnei
        ...
        if(cpu_map(father(igrid)).eq.myid) then ! if belong to myid
           ...
           !f_w_crit = max(0d0,f_w_crit)
            ...
            vload_rad = dsqrt(2d0*f_ESN*nSN_i*E_SNII*(1d0+f_w_crit)/(M_SN_var*msun2g))/(1d0+f_w_cell)/scale_v/f_LOAD/f_PCAN   
           ...
           if(f_w_cell.ge.f_w_crit)then ! radiative phase
              ...
              vload = dsqrt(2d0*f_esn2*nSN_i*E_SNII/(1d0+f_w_cell)/(M_SN_var*msun2g))/scale_v/f_LOAD
              !vload = dsqrt(2d0*f_esn2*E_SNII/(1d0+f_w_cell)/(M_SN_var*msun2g))/scale_v/f_LOAD
              if(mechanical_geen) then
