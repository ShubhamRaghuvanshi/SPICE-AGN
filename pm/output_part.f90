  ...
  if (star .or. sink) then
    ...
    if (metal) then
      ...
      if(is_oxygen) then
        ...
        call generic_dump("metallicityII", ivar, xdp, unit_out, dump_info, unit_info)
      endif
    end if ! if metal
    ...
    call generic_dump("imass", ivar, xdp, unit_out, dump_info, unit_info)
    deallocate(xdp)
    ...
    do i = 1, npartmax
      if (levelp(i) > 0) then
        ipart=ipart+1
        ll(ipart) = ptracegroup(i)
      end if
    end do
    call generic_dump("tracegroup", ivar, ll, unit_out, dump_info, unit_info)
    deallocate(ll)
#endif
  end if
