...
subroutine create_sink
  ...
  !if(verbose)
  if(myid == 1)write(*,*)' Entering create_sink', nsink
...
subroutine make_sink(ilevel)
  ...
  !if(myid==1)write(*,*)' Entering make_sink for level ',ilevel
  ...
  if (star)then
    ...
    !write(*,*) "star is disabled for now"
    !call clean_stop
  endif  
  ...
  bhspin_new=0d0; spinmag_new=0d0; Efeed_new=0d0; new_born_new=.false.
  ...
  !if(myid==1) write(*,*) "Convert hydro variables to primitive variables", nsink
  !------------------------------------------------
  ...
  !if(myid==1) write(*,*) "Compute number of new sinks", nsink, "rho_crit :", n_sink*1.6735575e-24
  !----------------------------
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,ngrid
           ...
           !Skip at very high redshift
           !if(aexp<1d-2)ok(i)=.false.
           if(aexp<0.051)ok(i)=.false.

           ! Check if the stellar density is higher than a star/sink formation threshold
           if(star) then 
             if(rho_star(ind_cell(i))<max(d_star, dd_sink)) then 
               ok(i)=.false.
               !write(*,*) myid, "Well star should still be off"
              endif 
           endif 
...
!!$        if(star.and.star_ratio<star_ratio_floor)ok(i)=.false.
           ...
           !shubh 
           ! Check if gas density exceeds the User defined threshold
           if(d < dd_sink)ok(i)=.false.
           !shubh 
           ...
           !if(myid ==1) write(*,*) "Check if the mass criterion will be met"
           if (force_exact_mseed) then
              if ((d-dd_sink/4.0)*dx_loc**3 < Mseed*2d33/scale_m) ok(i)=.false.
              !if ((d-d_jeans)*dx_loc**3 < Mseed*2d33/scale_m) ok(i)=.false.
           endif
           ! /ENFORCE MASS CRITERION

           if(ok(i))then 
             !write(*,*) "myid: ", myid, "creating sink, nsink :",nsink+1, "  z = ",1.0/aexp - 1.0 
        ...
        !if(myid==1) write(*,*) "Calculate number of new sinks in each cell plus cloud particles"
        do i=1,ngrid
           flag2(ind_cell(i))=0
           if(ok(i))then
              ntot=ntot+1
  ...
  ntot_tmp=ntot
#endif
  !write(*,*) "LOCAL Check multiple sink creation", myid, ntot
  !if(myid==1) write(*,*) "Check multiple sink creation", nsink, ntot_tmp
...
#endif
  !if(myid==1) write(*,*) "Checking pairs of distances ", ntot_tmp, "    ", ntot_tmp*(ntot_tmp-1)/2
  do i=1,ntot_tmp
  ...
  !---------------------------------
  !if(myid==1) write(*,*) "Check for free particle memory", nsink
  ...
  !if(myid==1) write(*,*) "Create new sink particles", nsink, ntot_all
  !------------------------------
  ! Create new sink particles
  !------------------------------
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           !if(myid==1) write(*,*) "mass of new sink :" , index_sink, msink_new(index_sink)*scale_m/2d33
           if(msink_new (index_sink) < 0 ) then 
            write(*,*) "NEGATIVE MASS WARNING : Mass of the new sink ", d, d_thres,  dx_loc**3, Mseed, scale_m
           endif 
  ...
  !if(myid==1) write(*,*) "Convert hydro variables back to conservative variables", nsink
  !---------------------------------------------------------
  ...
  !/Particles dynamical friction (HP)
#endif
 !if(myid==1) write(*,*) "Particles dynamical friction (HP)", nsink
  do isink=1,nsink
     if(oksink_all(isink)==1)then
        tsink(isink) =tsink_all(isink)
        idsink(isink)=idsink_all(isink)
        msink(isink) =msink_all(isink)
        if(msink(isink) < 0 .and. myid == 1) then 
          write(*,*) "NEGATIVE WARNING : Particles dynamical friction (HP)"
        endif
...
if (MC_tracer) then
  ...
  do isink=1,nsink
     ...
     if(msink_new(igrp) < 0 ) then 
     write(*,*) "NEGATIVE WARNING:  HP" , msink_new(igrp),msink (isink)
     endif 
  ...
  !if(myid==1) then 
  !do isink=1,nsink
  !  if(msink_new(isink) > msink(isink) ) then 
  !    write(*,*) "merging sink mass :", 1.0/aexp - 1.0, isink, msink_new(isink) - msink(isink)
  !  endif 
  !enddo  
  !endif 
  !shubh
...
subroutine create_cloud_from_sink
  ...
  real(dp)::scale,dx_min,rr,rmax,dx_cloud
...
#ifndef WITHOUTMPI
  integer ::info
  integer ::nsink_min,nsink_max,cloud_counter
#endif
  ! Conversion factor from user units to cgs units
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  ...
  do isink=1,nsink
     !if(bhzoom_ncells>0)then
     !   dx_cloud=scale*0.5d0**(nlevelmax_current)
     !else
        dx_cloud=scale*0.5d0**(levelsink(isink))
     !endif
     rmax=dble(ir_cloud)*dx_cloud
     do kk=-2*ir_cloud,2*ir_cloud
        xrel(3)=dble(kk)*dx_cloud/2.0
        do jj=-2*ir_cloud,2*ir_cloud
           xrel(2)=dble(jj)*dx_cloud/2.0
           do ii=-2*ir_cloud,2*ir_cloud
              xrel(1)=dble(ii)*dx_cloud/2.0
              rr=sqrt(sum(xrel**2))
              if(rr<=rmax)then
                 ...
                 if(cc(1).eq.myid)then
                    ...
                    if(mass_sink_direct_force .ge. 0.0)then
                       if(msink(isink)<mass_sink_direct_force*2d33/(scale_d*scale_l**ndim))then
                          !Below threshold
                          if(point_mass_sink)then
                             if((ii.eq.0).and.(jj.eq.0).and.(kk.eq.0))then
                                mp(indp)=msink(isink) !All the mass on the central cloud
                             else
                                mp(indp)=0   !No mass on the outer clouds
                             endif
                          else
                             mp(indp)=msink(isink)/dble(ncloud_sink)  !Distribute sink mass over clouds
                          endif
                       else
                          !Direct force case
                          mp(indp)=0
...
subroutine bondi_hoyle(ilevel)
  ...
  !if(myid==1) write(*,*) "Entering bondi_Hoyle for level : ", ilevel
  ...
  if(nsink>0)then
     ...
     !shubh_bhrate
     call MPI_ALLREDUCE(wBHrate,wBHrate_new,nsinkmax,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     !shubh_bhrate
  ...
  do isink=1,nsink
     ...
     enddo
     !shubh_bhrate
     weighted_BHrate(isink,ilevel)=wBHrate_new(isink)
     !shubh_bhrate
  end do
...
subroutine average_density(ind_grid,ind_part,ind_grid_part,ng,np,ilevel)
  ...
  !shubh_bhrate
  real(dp),dimension(1:nvector),save::BHrategas   
  !shubh_bhrate
 ...
 ! Compute parent cell position
  ...
  do ind=1,twotondim
     do j=1,np
        ...
        !shubh_bhrate
        BHrategas(j) = BHrategas(j) + d* (d/(c2+v*v)**1.5d0) *vol(j,ind)
        !shubh_bhrate
     end do
  end do
  ...
  do j=1,np
     ...
     !shubh_bhrate
     wBHrate(isink)=wBHrate(isink) + weight * BHrategas(j)
     !shubh_bhrate
...
subroutine grow_bondi(ilevel)
  ...
  real(dp)::alpha,d_star,pi,factG,c2mean,nfloor,sigmav2,v2mean,BHratemean
  ...
  if(numbtot(1,ilevel)==0)return
  if(verbose)write(*,111)ilevel
  !if(myid==1) write(*,*) "entering grow_bondi for level :", ilevel 
  ...
  ! Reset new sink variables
  msink_new=0d0; vsink_new=0d0; dMBH_coarse_new=0d0; dMEd_coarse_new=0d0; dMsmbh_new=0d0; acc_mom=0d0
  ! AGNRT
#ifdef SINKAGN_RT
  if (rt .and. rt_AGN) dMeff_coarse_new = 0.d0
#endif
  ...
  do isink=1,nsink
     !Check levelsink
     if(levelsink(isink).lt.ilevel)then
        levelsink(isink)=ilevel
     endif
     ...
     do i=levelmin,nlevelmax
        ...
        !shubh
        BHratemean =BHratemean +weighted_BHrate(isink,i)
        !shubh
     ...
     !shubh_bhrate
     BHratemean = BHratemean /volume/density
     !shubh_bhrate
     ! --------------------------
     ...
     !alpha=1d0
     !shubh
     v2mean_sink(isink) = v2mean
     c2mean_sink(isink) = c2mean
     dmean_sink(isink)  = density   
     !shubh
     ...
     if(Esave(isink).eq.0d0)then
        ...
        !     & / (c2mean+v2mean)**1.5d0
        !shubh_bhrate
        !dMBHoverdt(isink)=alpha * fourpi * (factG*msink(isink))**2 *  &
        !     & density / (c2mean+v2mean)**1.5d0
        dMBHoverdt(isink)=alpha * fourpi * (factG*msink(isink))**2 *  &
             & BHratemean
        !if(isink.eq.1 .and. myid.eq.1) then 
        !write(*,*) "c2mean, v2mean, ilevel :",c2mean, v2mean, ilevel 
        !endif      
        !shubh_bhrate     
     else
  ...
  if(nsink>0)then
     ...
     call MPI_ALLREDUCE(msink_new,msink_all,nsinkmax     ,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     call MPI_ALLREDUCE(vsink_new,vsink_all,nsinkmax*ndim,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     call MPI_ALLREDUCE(acc_mom,acc_mom_all,nsinkmax*ndim,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     ...
     call MPI_ALLREDUCE(dMsmbh_new     ,dMsmbh_all     ,nsinkmax,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
     call MPI_ALLREDUCE(levelsink, levelsink_all, nsinkmax, MPI_INT, MPI_MAX,  MPI_COMM_WORLD,info);
! AGNRT
#ifdef SINKAGN_RT
     if(rt .and. rt_AGN) call MPI_ALLREDUCE(dMeff_coarse_new,dMeff_coarse_all,nsinkmax,MPI_DOUBLE_PRECISION,MPI_SUM,MPI_COMM_WORLD,info)
#endif
!/AGNRT
#else
     msink_all=msink_new
     ...
     levelsink_all=levelsink
#ifdef SINKAGN_RT
     if(rt .and. rt_AGN) dMeff_coarse_all = dMeff_coarse_new
#endif
#endif
  endif

  do isink=1,nsink
     levelsink(isink)=levelsink_all(isink)
     if((.not.fix_smbh_position).and.accrete_momentum)then
     ...
     !shubh
#ifdef SINKAGN_RT
     if(rt .and. rt_AGN) dMeff_coarse(isink) = dMeff_coarse(isink) + dMeff_coarse_all(isink)
#endif
  end do
 ...
 ! Lower left corner of 3x3x3 grid-cube
  ...
  do j=1,np
     if(ok(j))then
        ...
        if (no_accretion.or.levelsink(isink)<nlevelmax_current) then
           acc_mass = 0d0
...
! AGNRT
#ifdef SINKAGN_RT
        if (rt .and. rt_AGN) dMeff_coarse_new(isink) = dMeff_coarse_new     (isink) + dmsink
#endif
!/AGNRT

        msink_new(isink  )=msink_new(isink  )+dmsink
        vsink_new(isink,1)=vsink_new(isink,1)+dmsink*u
        acc_mom(isink,1)=acc_mom(isink,1)+dmsink*u
#if NDIM>1
        vsink_new(isink,2)=vsink_new(isink,2)+dmsink*v
        acc_mom(isink,2)=acc_mom(isink,2)+dmsink*v
#endif
#if NDIM>2
        vsink_new(isink,3)=vsink_new(isink,3)+dmsink*w
        acc_mom(isink,3)=acc_mom(isink,3)+dmsink*w
#endif
        ...
        if(drag .and. rg_scale(isink) <= dx_min*0.2)then
           ...
           do while ((Deltat .lt. dtnew(ilevel)) .and. (counter .le. 10))
              ...
              !Numerical effects for extremely small relative velocities. Simply skip updating the BH velocity
              if (vnorm_rel>2*smallc)then
                 do idim=1,ndim
              ...
              if (levelsink(isink).eq.nlevelmax_current) then
                 ekk=0.0d0
                 do idim=1,ndim
...
subroutine grow_jeans(ilevel)
  ...
  if(numbtot(1,ilevel)==0)return
  if(verbose)write(*,111)ilevel
  !if(myid==1) write(*,*) "Entering grow_jeans for level :", ilevel
...
subroutine growspin
  ...
  do isink=1,nsink
     ...
     ay2=jsink(isink,2)
     az2=jsink(isink,3)
     jsinknorm=max(SQRT(ax2**2+ay2**2+az2**2),smallr)
     ax2=ax2/jsinknorm
     ay2=ay2/jsinknorm
     az2=az2/jsinknorm
...
subroutine AGN_feedback
  ...
  real(dp),dimension(:),allocatable::dMBH_AGN,dMEd_AGN,dMsmbh_AGN,EsaveAGN,EfeedAGN,Msmbh,spinmagAGN,eps_AGN,X_radio
...
!!$  call MPI_BARRIER(MPI_COMM_WORLD,info)
  ...
  allocate(xAGN(1:nAGN,1:3),vAGN(1:nAGN,1:3),mAGN(1:nAGN),dAGNcell(1:nAGN),iAGN_myid(1:nAGN) &
       & ,dMBH_AGN(1:nAGN),dMEd_AGN(1:nAGN),dMsmbh_AGN(1:nAGN),Msmbh(1:nAGN),EsaveAGN(1:nAGN),EfeedAGN(1:nAGN),jAGN(1:nAGN,1:3) &
       & ,spinmagAGN(1:nAGN),eps_AGN(1:nAGN), X_radio(1:nAGN))
  allocate(passiveAGN(1:nAGN,imetal:nvar))
  xAGN=0; vAGN=0; mAGN=0d0; dAGNcell=0; iAGN_myid=0; dMBH_AGN=0d0; dMEd_AGN=0d0; EsaveAGN=0d0; EfeedAGN=0d0; jAGN=0d0; spinmagAGN=0; eps_AGN=0; X_radio(1:nAGN)=0; passiveAGN = 0
  ...
  call AGN_blast(xAGN, vAGN, dMsmbh_AGN, dMBH_AGN, dMEd_AGN, mAGN,    &
       ...
       nAGN, iAGN_myid, ok_blast_agn, EsaveAGN, EfeedAGN, spinmagAGN, eps_AGN, X_radio)
  ...
  ! AGNRT
#ifdef SINKAGN_RT
  if(rt .and. rt_AGN) dMeff_coarse = 0.d0
#endif
  !/AGNRT
  ...
  deallocate(iAGN_myid,ok_blast_agn,dMBH_AGN,dMEd_AGN,dMsmbh_AGN,Msmbh,EsaveAGN,EfeedAGN)
...
subroutine AGN_blast(xAGN,vAGN,dMsmbh_AGN,dMBH_AGN,dMEd_AGN,mAGN,dAGNcell,passiveAGN,jAGN,ind_blast,vol_gas &
     & ,psy_norm,vol_blast,nAGN,iAGN_myid,ok_blast_AGN,EsaveAGN,EfeedAGN,spinmagAGN,eps_AGN, X_radio)
  use pm_commons
  use hydro_commons
  use cooling_module, only: XH=>X, rhoc, mH
  ...
  real(dp), dimension(1:nAGN), intent(inout) :: EsaveAGN, EfeedAGN, vol_gas
...
!!$        epsilon_r=1d0-sqrt(1d0-2d0/(3d0*r_lso))
     ...
     EfeedAGN(iAGN)=EAGN(iAGN)/((3d10/scale_v)**2d0)
  end do
  EsaveAGN=0d0
...
subroutine getAGNonmyid(isink_myid,nsink_myid)
  ...
  integer ::ii
  integer(kind=8),dimension(1:8)::idom,jdom,kdom
  integer,dimension(1:8)::cpu_min,cpu_max
...
subroutine update_sink(ilevel)
  ...
  integer::lev,isink,isink_2,nx_loc,idim
  ...
  !Skipping bit about sink merger here. Add later?
  if(verbose)write(*,*)"NCLOUD first sink in update_sink_position_velocity:",sum(sink_stat_all(1,levelmin:nlevelmax,7))
  ...
  do isink=1,nsink
     if(msink(isink)>0.0)then
        ...
        ! Forces in non direct force case are summed over all clouds
       ...
       ! Compute timestep for the synchronization
        if (new_born(isink))then
           ...
           if(levelsink(isink)>ilevel)then
...
subroutine f_gas_sink(ilevel)
  ...
  use hydro_commons
  use poisson_commons
  use cooling_module, ONLY: pi, twopi
  use mpi_mod
  implicit none
     ...
     if (direct_force_sink(isink))then
        ...
        do igrid=1,ncache,nvector
           ...
           do ind=1,twotondim
              ...
              ! Gas and dark matter mass in cell => should include analytic mass! 
              do i=1,ngrid
  ...
  !write(*,*)myid,ilevel,fsink_new(1,1:ndim)
  ...
  !f contains the acceleration of the gas due to the sink
  do idim=1,ndim
     ...
     if(msink(isink)>0.0)then
        if (direct_force_sink(isink))then
           ...
           do idim=1,ndim
              ...
              if (period(idim))then
                 do ksink=1,nsink
                    if (direct_force_sink(ksink))then
                       ff(ksink,idim)=xsink(ksink,idim)-xsink(isink,idim)
                       if(ff(ksink,idim)>0.5d0*boxlen)ff(ksink,idim)=ff(ksink,idim)-boxlen
                       if(ff(ksink,idim)<-0.5d0*boxlen)ff(ksink,idim)=ff(ksink,idim)+boxlen
                       d2(ksink)=d2(ksink)+ff(ksink,idim)**2
                    end if
                 end do
              else
                 do ksink=1,nsink
           ...
           do ksink=1,nsink
              if (direct_force_sink(ksink))then
                 ff(ksink,1:ndim)=factG*msink(ksink)/(ssoft**2+d2(ksink))**1.5d0*ff(ksink,1:ndim)
              end if
           end do
           do ksink=1,nsink
              fsink(isink,1:ndim)=fsink(isink,1:ndim)+ff(ksink,1:ndim)
              if(ksink<0)then
...
!################################################################
!################################################################
subroutine calculate_sink_levels
use amr_commons
use pm_commons
use mpi_mod
implicit none
...
integer::ilevel
do ilevel=levelmin,nlevelmax
   if(numbtot(1,ilevel)>0)levelsink=ilevel
enddo
     ...
     levelsink_all=levelsink
#endif
  levelsink=levelsink_all

end subroutine calculate_sink_levels
...
subroutine read_sink_params()
  ...
  namelist/sink_params/agn,smbh,sink_AGN,bondi,drag,spin_bh,force_exact_mseed &
    ...
    & ,point_mass_sink,output_finestep_sink,accrete_momentum &
    & ,ir_cloud,mass_sink_direct_force,sink_soft &
    & ,sink_descent,gamma_grad_descent,fudge_graddescent & 
    & ,nsinkmax 
...
! namelist sanity checks
if (.not. sink) then 
  if(myid==1)write(*,*) 'what are you even doing here'
  call clean_stop
end if 

if (n_sink<0.0d0) then
  if(myid==1)write(*,*)'Error in the namelist' 
  if(myid==1)write(*,*) 'n_sink is not set'
  call clean_stop
end if 
...
!-----------------------------------
!Error checking for BH zoom 
!----------------------------------

!*** incorporate these changes in refine params in read_hydro_params

!Number of levels reserved for the BH
!if(bhzoom_level>0.and.bhzoom_ncells>0)then
!  bhzoom_leveldiff=nlevelmax-bhzoom_level
!endif

!if((bhzoom_ncells<ir_cloud).and.(bhzoom_ncells>0))then
!  if(myid==1)write(*,*)"bhzoom_ncells has to be larger than the extent of the cloud particles"
!  if(myid==1)write(*,*)"Setting bhzoom_ncells equal to",ir_cloud
!  bhzoom_ncells=MAX(bhzoom_ncells,REAL(ir_cloud))
!endif

!f(myid==1)write(*,*)"Leveldiff",bhzoom_leveldiff
!f((bhzoom_ncells>0.and.bhzoom_level<0).or.(bhzoom_ncells<0.and.bhzoom_level>0))then
!  if(myid==1)then
!     write(*,*)"Please set both bhzoom_ncells (number of cells in zoom region radius)"
!     write(*,*)"and bhzoom_level (the level outside the bzoom region)"
!  endif
!  call clean_stop
!endif
!if(bhzoom_level>nlevelmax)then
!  if(myid==1)then
!     write(*,*)"bhzoom_level cannot be larger levelmax"
!  endif
!  call clean_stop
!endif
