...
subroutine init_sink
  ...
  allocate(levelsink(1:nsinkmax))
  allocate(levelsink_all(1:nsinkmax))
  ...
  allocate(dvsink(1:nsinkmax,1:ndim))
  allocate(xsink(1:nsinkmax,1:ndim))
  allocate(vsink_new(1:nsinkmax,1:ndim))
  allocate(vsink_all(1:nsinkmax,1:ndim))
  allocate(acc_mom(1:nsinkmax,1:ndim))
  allocate(acc_mom_all(1:nsinkmax,1:ndim))
...
!shubh_bhrate
  allocate(weighted_BHrate(1:nsinkmax,1:nlevelmax))
  allocate(wBHrate(1:nsinkmax))
  allocate(wBHrate_new(1:nsinkmax))
!shubh_bhrate
...
!shubh
  allocate(dMBHoverdt_out  (1:nsinkmax))
  allocate(dMEddoverdt_out (1:nsinkmax))
  allocate(dMsmbhoverdt_out(1:nsinkmax))
  allocate(v2mean_sink(1:nsinkmax))
  allocate(c2mean_sink(1:nsinkmax))
  allocate(dmean_sink(1:nsinkmax))
  allocate(vol_agn(1:nsinkmax))
...
!shubh
  ...
  spinmag_new=0; spinmag_all=0; eps_sink=0; levelsink=0
  direct_force_sink=.false.;
  ...
  if(nrestart>0)then
     ...
     if (ic_sink) then
        ...
        if(nsink>0)then
           ...
           !/AGNRT
           deallocate(xdp)
           new_born(1:nsink)=.false. ! this is a restart
        end if
        close(ilun)
        ! Send the token
#ifndef WITHOUTMPI
  ...
  if (ic_sink.and.read_ic_sink)then
#ifndef WITHOUTMPI
     if(IOGROUPSIZE>0) then
        if (mod(myid-1,IOGROUPSIZE)/=0) then
           dummy_io=1
     ...
     ! Every other line: sink mass [Msun], x , y, z, vx [code_vel], vy [code_vel], vz [code_vel], lx, ly, lz
     ...
     do isink=1, nsinknew
        ...
        xsink(nindsink,1)=xx1*boxlen
        xsink(nindsink,2)=xx2*boxlen
        xsink(nindsink,3)=xx3*boxlen
        vsink(nindsink,1)=vv1!*1d5/scale_v
...
102  continue
  ...
  if(output_finestep_sink)then
     do isink=1,nsink
        call create_sink_fine_file(idsink(isink))  !create file forsinkfile fine info
     end do
  endif
