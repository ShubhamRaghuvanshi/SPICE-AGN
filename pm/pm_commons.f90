module pm_commons
  ...
  !shubh_bhrate
  real(dp), allocatable, dimension(:,:) :: weighted_BHrate
  real(dp), allocatable, dimension(:) :: wBHrate, wBHrate_new
  !shubh_bhrate
  ...
  real(dp), allocatable, dimension(:,:) :: dvsink
  ...
  real(dp), allocatable, dimension(:,:) :: acc_mom,acc_mom_all
...
!shubh
  real(dp), allocatable, dimension(:) :: dMsmbhoverdt_out, dMEddoverdt_out, dMBHoverdt_out
  real(dp), allocatable, dimension(:) :: v2mean_sink,c2mean_sink,dmean_sink,vol_agn
!shubh
  ...
  integer , allocatable, dimension(:) :: levelsink,levelsink_all
  !Direct force sink
  ...
  integer::ncloud_sink,ncloud_sink_massive                       !Number of cloud particles
  integer::drag_level=0
  integer::drag_level_all=0
  !/Particles dynamical friction (HP)
  integer :: nindsink = 0
  ...
  !real(dp),allocatable,dimension(:)    ::weth
  !real(dp),allocatable,dimension(:)    ::wden,wvol,wdiv,wden_new,weth_new,wvol_new,wdiv_new
  !real(dp),allocatable,dimension(:,:)  ::wmom,wmom_new
  !real(dp),allocatable,dimension(:,:)  ::vsink,vsink_new,vsink_all
  !real(dp),allocatable,dimension(:,:)  ::fsink,fsink_new,fsink_all
  !real(dp),allocatable,dimension(:,:,:)::vsnew,vsold
  ...
  !tracer 
#ifdef MC_tracer
  integer,  allocatable, dimension(:)   :: tmpp    ! Working array
