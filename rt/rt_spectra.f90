...
!_________________________________________________________________________
...
#ifdef SINKAGN_RT
  !sinkagn_rt
     , init_AGNSED_table, inp_AGNSED_table, getAGNfraclum               & 
  !sinkagn_rt
#endif 

#ifdef NTRACEGROUPS
      , update_star_RT_feedback, star_RT_feedback, get_tracer_group_from_halo 
#else
  ...
  real(kind=8),allocatable,dimension(:)::SED_ages,SED_zeds![Gyr],[m_met/m_gas]
  ...
  !sinkagn_rt
#ifdef SINKAGN_RT  
  ! Same for the AGN SED properties
  real(kind=8)::AGNSED_dlgM, AGNSED_dlgE
  real(kind=8)::AGNSED_lgM0, AGNSED_lgE0
  integer::AGNSED_nmass, AGNSED_nedd    ! Number of mass bins and edd bins
  real(kind=8),allocatable,dimension(:)::AGNSED_masses                ! [Msun]
  real(kind=8),allocatable,dimension(:)::AGNSED_edd
  ! AGNSED_table: iMasses, iEdd, igroups, properties
  !                                       (lumfrac, egy, csn, cse).
  real(kind=8),allocatable,dimension(:,:,:,:)::AGNSED_table
#endif 
  !sinkagn_rt
...
#ifdef NTRACEGROUPS
  character(len=256)::hZs, hhprops                             ! Filenames
  logical::okhz, okhprop
  integer:: hnzs, hnhs, hncs, ih
  real(kind=8) :: adiff, adiffnew
...
!initialize halo properties for photon tracers
#ifdef NTRACEGROUPS
  if(ntracegroups.gt.0) then
     if (rt_tracemassbins(1).gt.-1.d0) then 
        if(myid==1) &
              write(*,*) 'Tracer groups > 0, so initializing halo properties'
        write(hZs,'(a,a)')   trim(initfile(levelmin)),"/halo_redshifts.dat"
        inquire(file=hZs, exist=okhz)
        if(.not. okhz ) then
           if(myid.eq.1) then 
              write(*,*) 'Cannot read halo redshifts file...'
              write(*,*) 'Check if SED-directory contains the files ',  &
                         'halo_redshifts.dat'
           endif
           call clean_stop
...
!Read halo properties from tracers
#ifdef NTRACEGROUPS
  if (rt_tracemassbins(1).gt.-1.d0) then 
     !READ REDSHIFTS
     open(unit=10,file=hZs,status='old',action='read')
     read(10,*) hnzs
     allocate(halopropz(hnzs))
     do ih = 1, hnzs
        read(10,*) halopropz(ih)
     end do
     close(10)

     !READ HALO PROPERTIES FOR THE CORRECT REDSHIFT

     !first get the right redshift
     ...
     enddo
     !now read in the correct file (we want minind_temp-1 because catalogs start at 000)
     write(hhprops,'(a,a,I0.3,a)') trim(initfile(levelmin)),"/halo_files/ramses_halo_properties_",minind_temp-1,".dat"
     if(myid==1) write(*,*) hhprops
...
!sinkagn_rt
! MT: also include AGN radiation. Note that contrary to stars, we assume
! that for AGN radiation, cross-section and average energy is fixed.
! For the three ionizing bands:
! - group_egy_agn [eV]          = 18.29, 35.63, 128.3
! - group_cse_agn(HI)   [cm**2] = 2.990e-18, 4.663e-19, 2.148e-20
! - group_cse_agn(HeI)  [cm**2] =         0, 3.993e-18, 3.646e-19
! - group_cse_agn(HeII) [cm**2] =         0,         0, 3.021e-19
! - group_csn_agn(HI)   [cm**2] = 3.181e-18, 5.094e-19, 3.877e-20
! - group_csn_agn(HeI)  [cm**2] =         0, 4.235e-18, 6.349e-19
! - group_csn_agn(HeII) [cm**2] =         0,         0, 5.359e-19
!-------------------------------------------------------------------------
!sinkagn_rt
...
#endif
  integer :: i, ip, ii
  real(kind=8),save,allocatable,dimension(:)::  L_star
  real(kind=8),save,allocatable,dimension(:,:)::csn_star, cse_star
  real(kind=8),save,allocatable,dimension(:)::  egy_star
  real(kind=8),save,allocatable,dimension(:)::  sum_L_cpu,sum_L_all
  real(kind=8),save,allocatable,dimension(:,:)::sum_csn_cpu,sum_csn_all
  real(kind=8),save,allocatable,dimension(:,:)::sum_cse_cpu,sum_cse_all
  real(kind=8),save,allocatable,dimension(:)::sum_egy_cpu,sum_egy_all
  real(kind=8):: mass, age, Z, t_sne_Gyr

  !sinkagn_rt
#ifdef SINKAGN_RT
  real(kind=8):: sum_LAGN(1:nSEDgroups)
  real(kind=8):: scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  real(kind=8):: scale_evtocode, scale_vol, scale_np, scale_fp, scale_msun
#endif
  !sinkagn_rt
...
!-------------------------------------------------------------------------
  ...
  !sinkagn_rt
#ifdef SINKAGN_RT
  sum_LAGN = 0._dp  ! For AGN radiation
#endif
  !sinkagn_rt
  ...
  !sinkagn_rt
#ifdef SINKAGN_RT
  ! Get the total luminosity in each band for the AGN radiation
  if (rt .and. rt_AGN) then
    do ip=1, nSEDgroups
      ! sum_LAGN should be in [# photons s-1]
      sum_LAGN(ip) = sum(LAGN_coarse(:)*lumfrac_AGN(:,ip))
      sum_LAGN(ip) = sum_LAGN(ip) * (scale_vol * scale_np)  ! [dM c**2 in eV] / dtcoarse
      sum_LAGN(ip) = sum_LAGN(ip) /group_egy_agn(ip)        ! # photons per dtcoarse
      sum_LAGN(ip) = sum_LAGN(ip) / scale_t                 ! # photons s-1
        ! Note: there might be some initialisation issue, but seems consistent.
     end do
  end if
#endif 
  !sinkagn_rt
  ...
  do ip=1,nSEDgroups
    ...
    !sinkagn_rt
#ifdef SINKAGN_RT
    ! Do not update the SED if we want it fixed...
    if(.not. group_sed_update(ip)) cycle
#endif 
    !sinkagn_rt  

  !sinkagn_rt 
#ifdef SINKAGN_RT
    if(sum_L_all(ip) .gt. 0. .or. sum_LAGN(ip) .gt. 0._dp) then
      group_egy(ip) = sum_egy_all(ip) + group_egy_agn(ip)*sum_LAGN(ip) 
      group_egy(ip) = group_egy(ip)/(sum_L_all(ip) + sum_LAGN(ip))

      group_csn(ip,:) = sum_csn_all(ip,:) + group_csn_agn(ip,:)*sum_LAGN(ip) 
      group_csn(ip,:) = group_csn(ip,:)/(sum_L_all(ip) + sum_LAGN(ip))

      group_cse(ip,:) = sum_cse_all(ip,:) + group_cse_agn(ip,:)*sum_LAGN(ip)
      group_cse(ip,:) = group_cse(ip,:)/(sum_L_all(ip) + sum_LAGN(ip))
...
!sinkagn_rt
#ifdef SINKAGN_RT
SUBROUTINE init_AGNSED_table()

! Initiate AGN SED properties table, which gives luminosity fractions of
! each photon group as a function of the BH mass and accretion rate.
! The SED is read from a directory specified by sed_dir.
!-------------------------------------------------------------------------
  use amr_commons,only:myid
  use rt_parameters
  use spectrum_integrator_module
...
#endif
  ! Temporary SSP/SED parameters (read from SED files):
  integer:: nmass, nedd ! # of bins of mass, Edd ratio and groups
  real(kind=8),allocatable::mass_bins(:), edd_bins(:)
  real(kind=8),allocatable::fraclum(:,:,:)   ! fraction (edd, mass, group)
  !real(kind=8),allocatable::tbl(:,:,:), reb_tbl(:,:,:)
  integer::i,imass,ip
  character(len=128)::fEdds, fMasses, fSEDs                    ! Filenames
  logical::ok,okMass,okEdd
  !real(kind=8)::pL0, pL1, tmp
  !integer::nv=3+2*nIons  ! # vars in SED table: L,Lacc,egy,nions*(csn,egy)
  integer,parameter::tag=1134
!-------------------------------------------------------------------------
  if(myid==1) &
        write(*,*) 'AGNs are photon emitting, so initializing AGNSED table'
  if(SED_DIR.eq.'')call get_environment_variable('RAMSES_SED_DIR',SED_DIR)
  inquire(FILE=TRIM(sed_dir)//'/agnseds.dat', exist=ok)
  if(.not. ok)then
     if(myid.eq.1) then
        write(*,*)'Cannot access SED directory ',TRIM(sed_dir)
        write(*,*)'Directory '//TRIM(sed_dir)//' not found'
  ...
  write(fEdds,'(a,a)')   trim(sed_dir),"/edd_bins.dat"
  write(fMasses,'(a,a)') trim(sed_dir),"/mass_bins.dat"
  write(fSEDs,'(a,a)') trim(sed_dir),"/agnseds.dat"
  inquire(file=fEdds, exist=okEdd)
  inquire(file=fMasses, exist=okMass)
  inquire(file=fSEDs, exist=ok)
  if(.not. ok .or. .not. okMass .or. .not. okEdd) then
     if(myid.eq.1) then
        write(*,*) 'Cannot read SED files...'
        write(*,*) 'Check if SED-directory contains the files ',  &
                   'edd_bins.dat, mass_bins.dat, agnseds.dat'
     endif
     call clean_stop
  end if
  ...
  ! READ METALLICITY BINS-------------------------------------------------
  open(unit=10,file=fEdds,status='old',form='formatted')
  read(10,'(i8)') nedd
  allocate(edd_bins(nedd))
  do i = 1, nedd
     read(10,'(e14.3)') edd_bins(i)
  end do
  close(10)
  ! READ AGE BINS---------------------------------------------------------
  open(unit=10,file=fMasses,status='old',form='formatted')
  ! Should be in Msun
  read(10,'(i8)') nmass
  allocate(mass_bins(nmass))
  do i = 1, nmass
     read(10,'(e14.3)') mass_bins(i)
  end do
  close(10)
  ! READ SEDS-------------------------------------------------------------
  open(unit=10,file=fSEDs,status='old',form='unformatted')
  ! FIXME
!!$  read(10,'(i8)') ngrpsed
!!$  if (ngrpsed /= nGroups) then
!!$     if (myid==1) write(*,*) 'Error in AGN SED: ngrpsed != ngroups', ngrpsed, nGroups
!!$     call clean_stop
!!$  end if
  !/FIXME
  allocate(fraclum(nedd,nmass,nGroups))
  do ip = 1, nGroups
     do imass = 1, nmass
        read(10) fraclum(:,imass,ip)
     end do
  end do
  close(10)
  ...
  ! FIXME: So far, only support pure SED groups (no UVB groups)
  if (nSEDgroups /= nGroups) then
     if (myid==1) write(*,*) 'Error in AGN SED: nSEDgroups != ngroups', nSEDgroups, nGroups
     call clean_stop
  end if



  ! FIXME: So far, only 1 property (frac_lum, fraction of Lbol)
  allocate(AGNSED_table(nmass, nedd, nSEDgroups, 1))
  allocate(AGNSED_masses(nmass))
  allocate(AGNSED_edd(nedd))
  AGNSED_nmass = nmass
  AGNSED_nedd = nedd

  ! This is in case we want to automatically rebin later
  AGNSED_edd(:) = edd_bins(:)
  AGNSED_masses(:) = mass_bins(:)
  
  do ip = 1,nSEDgroups                                ! Loop photon groups
     do imass = 1, nmass
        AGNSED_table(imass,:,ip,1) = fraclum(:,imass,ip)
     end do
  end do ! End photon group loop

  AGNSED_lgE0 = minval(log10(edd_bins))
  AGNSED_lgM0 = minval(log10(mass_bins))
  AGNSED_dlgE = log10(maxval(edd_bins)/minval(edd_bins)) / dble(nedd-1)
  AGNSED_dlgM = log10(maxval(mass_bins)/minval(mass_bins)) / dble(nmass-1)

  deallocate(fraclum)
  deallocate(mass_bins)
  deallocate(edd_bins)

  !if (myid==1) call write_AGNSEDtable

END SUBROUTINE init_AGNSED_table

SUBROUTINE inp_AGNSED_table(mass, edd, nProp, same, ret)
...
!          1=fraction of Lbol in each band
! same  => If true then assume same mass and Edd as used in last call.
...
!-------------------------------------------------------------------------
  use amr_commons
  use rt_parameters
  real(kind=8), intent(in):: mass, edd
  real(kind=8):: lgMass, lgEdd
  integer:: nProp
  logical:: same
  real(kind=8),dimension(:):: ret
  integer,save:: iM, iE
  real(kind=8),save:: dM, dM0, dM1, dE, dE0, dE1
!-------------------------------------------------------------------------
  if(.not. same) then
     if(Edd.gt.0) then
        lgEdd = log10(Edd)
     ...
     iM = min(max(floor((lgMass-AGNSED_lgM0)/AGNSED_dlgM )+1, 1  ),  AGNSED_nmass-1 )
     dM = AGNSED_masses(iM+1)-AGNSED_masses(iM)
     dM0= min( max(   (mass-AGNSED_masses(iM)) /dM,       0.d0 ), 1.d0          )
     dM1= min( max(  (AGNSED_masses(iM+1)-mass)/dM,       0.d0 ), 1.d0          )
     
     iE = min(max(floor((lgEdd-AGNSED_lgE0)/AGNSED_dlgE ) + 1,   1  ),  AGNSED_nedd-1 )
     dE = AGNSED_edd(iE+1)-AGNSED_edd(iE)
     dE0= min( max(   (Edd-AGNSED_edd(iE)) /dE,         0.d0 ),  1.d0         )
     dE1= min( max(  (AGNSED_edd(iE+1)-Edd)/dE,         0.d0 ),  1.d0         )
     
!!$     write(*,*)'Mass', mass, iM, mass_bins(iM), mass_bins(iM+1)
!!$     write(*,*)'Edd ', Edd, iE, edd_bins(iE), edd_bins(iE+1)
     
     if (abs(dM0+dM1-1.0d0) > 1.0d-5 .or. abs(dE0+dE1-1.0d0) > 1.0d-5) then
        write(*,*) 'Screwed up the sed interpolation ... '
        write(*,*) dM0+dM1,dE0+dE1
     end if
  endif
  
  ret = dM0 * dE0 * AGNSED_table(iM+1, iE+1, :, nProp) + &
        dM1 * dE0 * AGNSED_table(iM+1, iE,   :, nProp) + &
        dM0 * dE1 * AGNSED_table(iM, iE+1,   :, nProp) + &
        dM1 * dE1 * AGNSED_table(iM,   iE,   :, nProp)
  
END SUBROUTINE inp_AGNSED_table

SUBROUTINE getAGNfraclum(Mbh_Msun, Edd_ratio, ret)
...
!-------------------------------------------------------------------------
  use rt_parameters
  real(kind=8),intent(in):: Mbh_Msun, Edd_ratio
  real(kind=8),dimension(nSEDgroups):: ret
!-------------------------------------------------------------------------
  call inp_AGNSED_table(Mbh_Msun, Edd_ratio, 1, .false., ret)

  
END SUBROUTINE getAGNfraclum
#endif 
!sinkagn_rt
...
!-------------------------------------------------------------------------
  ...
  integer :: N,species
  real(kind=8),parameter :: const=1d8*hplanck*c_cgs/eV2erg  ! energy conv.
!-------------------------------------------------------------------------
  species      = 1                       ! irrelevant but must be included
...
!-------------------------------------------------------------------------
  ...
  real(kind=8),dimension(nSEDgroups):: ret, Lc0, Lc1
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  ! Grid based arrays
  real(kind=8),dimension(1:nvector,1:ndim),save::x0
  integer ,dimension(1:nvector),save::ind_cell
  integer ,dimension(1:nvector,1:threetondim),save::nbors_father_cells
  integer ,dimension(1:nvector,1:twotondim),save::nbors_father_grids
  ! Particle based arrays
  logical,dimension(1:nvector),save::ok
  real(kind=8),dimension(1:nvector,ngroups),save::part_NpInp
  real(kind=8),dimension(1:nvector,1:ndim),save::x
  ...
  real(kind=8)::age, z=0., scale_inp, scale_Nphot, dt_Gyr, dt_loc_Gyr    &
            , scale_msun, mass, t_sne_Gyr
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v, scale_Np  & 
            , scale_Fp
  real(kind=8),parameter::vol_factor=2**ndim   ! Vol factor for ilevel-1 cells
#ifdef NTRACEGROUPS
  integer::dumpid
  real(kind=8)::adiff,adiffnew,nptot
  real(kind=8),dimension(1:ntraceGroups)::npfrac
...
!-------------------------------------------------------------------------
  ...
  do j = 1, np
     ...
     ! For tracing photons based on stellar age 
#ifdef NTRACEGROUPS
     if(rt_traceagebins(1).gt.-1.d0) then
        dumpid = ntracegroups ! Start with the highest index
        do i=1,ntracegroups-1
           if ((age*1e3).le.rt_traceagebins(i)) dumpid=ntracegroups-i
        end do
        ptracegroup(ind_part(j)) = dumpid
     endif
#endif
...
!Get the tracer group id of the star particle
#ifdef NTRACEGROUPS 
     dumpid = 0
     if (rt_tracemassbins(1).gt.-1.d0) then
        if (file_trigger) then
           call get_tracer_group_from_halo(xp(ind_part(j),1:ndim),dumpid,minind)
           ptracegroup(ind_part(j)) = dumpid
        endif
        dumpid = ptracegroup(ind_part(j))
     endif

     !Note to harley, we want to put the metal function here as well
#endif
     if(.not. ok(j) ) &                                      ! ilevel cell
          part_NpInp(j,1:nSEDgroups)=part_NpInp(j,1:nSEDgroups)/vol_factor 
     do ip=1,nSEDgroups

#ifdef NTRACEGROUPS
        dumpid = ptracegroup(ind_part(j))
        hindex = ((iGroups(ip) - 1)/(nDim+1))*ntraceGroups
        hstart = (nGroups*(nDim+1)) + 1 !the generic starting index of the photon tracers 
        hstart = hstart+hindex
        hfin   = hstart+ntraceGroups-1
        npfrac = rtunew(indp(j),iGroups(ip)) * rtunew(indp(j),hstart:hfin) !this is the array of number of photons in each tracer group
        
        !total number of photons after update 
        nptot = rtunew(indp(j),iGroups(ip))+part_NpInp(j,ip)

        !add new photons to correct bin of the array
        npfrac(dumpid) = npfrac(dumpid)+part_NpInp(j,ip)
        
        !divide by the total number of photons after the injection to retireve the correct fractions
        rtunew(indp(j),hstart:hfin) = npfrac/nptot   
#endif
...
!*************************************************************************
#ifdef NTRACEGROUPS
SUBROUTINE get_tracer_group_from_halo(pos_star,groupindex,minind1)
  !This subroutine is called by star_RT_vsweep
  !It first identifies which halo catalog to load in.
  !It then calculates the distance of each star particle 
  !to the halos (accounting for periodic boundary conditions)
  !It then finds the closest halo with D < Rvir
  !If no halo exists with this condition, it chooses the closest halo 
  use amr_commons
  use pm_commons
  use rt_hydro_commons
  use rt_parameters
  real(kind=8),dimension(1:ndim)::pos_star,xdist
  integer::groupindex,minind1, hcount, i, mindcount, mindrcount
  real(kind=8)::distance,donrvir,mindist,minrdist

#ifndef NTRACEGROUPS
  return
#endif
  !starting index in the halo properties array
  !we use 5 because (mass,x,y,z,rvir)

  hcount = 1
  !Loop over properties, exit when no more haloes and we are at end of array
  mindist = 1d6
  minrdist = 1e6
  mindcount = 1
  mindrcount = 99999
  !Here we are looping over all of the haloes...I feel like there is probably a quicker way to do this
  !using the tree...if the halo and star particle count become very high, this may slow things down every time
  !a new file is read in.  This should only occur ~50-100 times per simulation so it shouldn't take too much time
  !but every time a new star particle forms we have to call this routine
  do while (hcount.le. size(haloprops(:,1))) 

     do i=1,ndim
        xdist(i) = abs(pos_star(i) - haloprops(hcount,i+1))
        if (xdist(i).gt.0.5d0) xdist(i) = 1.d0 - xdist(i) !accounting for periodic boundary conditions 
        xdist(i) = xdist(i)*xdist(i)                      !square the 1D distance
     end do
     distance = sqrt(sum(xdist))                    !3D distance from sqrt of the sum of squares
     donrvir = distance/haloprops(hcount,5) !distance/rvir
     ...
     !Find closest halo where particle is inside rvir
     if (donrvir.le.1.d0.and.distance.lt.minrdist) then
        minrdist = distance
        mindrcount = hcount
     end if
  ...
  groupindex = ntracegroups !Start at the highest index

  !if we have found the halo where the particle resides
  if (mindrcount.lt.99999) then
     do i=1,ntracegroups-1
        if (haloprops(mindrcount,1).le.rt_tracemassbins(i)) groupindex=ntracegroups-i
     end do
  !otherwise just choose nearest halo
  else
     write(*,*) 'star particle not in halo: d/rvir=',mindist/haloprops(mindcount,5)
     do i=1,ntracegroups-1
        if (haloprops(mindcount,1).le.rt_tracemassbins(i)) groupindex=ntracegroups-i
     end do
  end if

END SUBROUTINE get_tracer_group_from_halo
#endif
#endif
...
!_________________________________________________________________________
  ...
  real(kind=8),allocatable,dimension(:,:,:)::UV_groups_table
...
!-------------------------------------------------------------------------
  ...
  real(kind=8):: ret(nIons,2), dz0, dz1, zr_factor
  integer:: iz0, iz1
...
!-------------------------------------------------------------------------
  use amr_parameters
  real(kind=8), intent(in):: z
  real(kind=8):: ret(nUVgroups,2+2*nIons), dz0, dz1, zr_factor
  integer:: iz0, iz1
  logical, optional, intent (in) :: z_damp
!-------------------------------------------------------------------------
  ret=0. ; if(z .gt. UV_maxz) RETURN
...
!-------------------------------------------------------------------------
  ...
  implicit none
  integer::i
  real(kind=8),allocatable,save::UVprops(:,:) !Each group: flux, egy, csn, cse
...
!-------------------------------------------------------------------------
  ...
  integer :: N,species
  real(kind=8),parameter :: const=1d8*hplanck*c_cgs/eV2erg    ! unit conv.
!-------------------------------------------------------------------------
  species      = 1                       ! irrelevant but must be included
  norm         = integrateSpectrum(X, Y, N, e0, e1, species, f1)
  getUVEgy  = const * &
...
! Species is a code for the ion in question: 1=HI, 2=HeI, 3=HeIII
!-------------------------------------------------------------------------
  real(kind=8):: getUVcse, X(N), Y(N), e0, e1, norm
  integer :: N, species
!-------------------------------------------------------------------------
  if(e1 .gt. 0. .and. e1 .le. ionEvs(species)) then
