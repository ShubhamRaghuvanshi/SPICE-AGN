...
module rt_cooling_module
  ...
  real(kind=8)::T_min, T_frac, x_min, x_fm, x_frac
  real(kind=8)::Np_min, Np_frac, Fp_min, Fp_frac
  ...
  real(kind=8),dimension(nGroups)::kappaAbs=0! Dust absorption opacity    
  real(kind=8),dimension(nGroups)::kappaSc=0 ! Dust scattering opacity    
  ...
  real(kind=8),dimension(nGroups,nIons)::signc,sigec,PHrate

  real(kind=8),dimension(nIons, 2)::UVrates     !UV backgr. heating/ion. rates
...
!-------------------------------------------------------------------------
  ...
  if(cosmo) then
     call update_UVrates(dble(aexp))
     call init_coolrates_tables(dble(aexp))
  else
...
!-------------------------------------------------------------------------
  ...
  real(kind=8),dimension(1:nGroups, 1:nvector):: Np, dNpdt
  real(kind=8),dimension(1:ndim, 1:nGroups, 1:nvector):: Fp, dFpdt
  ...
  real(kind=8),dimension(nGroups):: dNp
  real(kind=8),dimension(1:ndim, 1:nGroups):: dFp
  ...
  real(kind=8)::one_over_rt_c_cgs, one_over_egy_IR_erg, one_over_x_FRAC
  real(kind=8)::one_over_Np_FRAC, one_over_Fp_FRAC, one_over_T_FRAC
  real(kind=8),dimension(1:nGroups) :: group_egy_ratio, group_egy_erg
...
contains
  ...
  !-----------------------------------------------------------------------
    ...
    real(kind=8),dimension(nDim),save:: dmom
    real(kind=8),dimension(nIons),save:: alpha, beta, nN, nI
    real(kind=8),save:: dUU, fracMax, x_tot
    real(kind=8),save:: mu, TK, nHe, ne, neInit, Hrate
    real(kind=8):: xHI, dxHI, xH2=0d0, dXH2=0d0, xHeI, dxHeI
    real(kind=8),save:: Crate, dCdT2, X_nHkb, rate, dRate, cr, de=0d0
    real(kind=8),save:: photoRate, metal_tot,metal_prime, ss_factor, f_dust
    integer,save:: iion,igroup,idim
    real(kind=8),dimension(nGroups),save:: recRad, phAbs, phSc, dustAbs
    real(kind=8),dimension(nGroups),save:: dustSc, kAbs_loc,kSc_loc
    real(kind=8),save:: rho, TR, one_over_C_v, E_rad, dE_T, fluxMag, mom_fact
    ...
    if (Zsolar(icell) .lt. 1.05 * z_ave) then
        f_dust = 1d-10
    endif    
    mu = getMu(dxion, dT2)
...
!------------------------------------------------------------------------
  ...
  real(kind=8),dimension(nGroups):: Np, dNp
  real(kind=8),dimension(nDim, nGroups):: Fp, dFp
...
!-------------------------------------------------------------------------
  ...
  real(kind=8)::aexp,daexp=0.,dt_cool,T2_com, nH_com  
  real(kind=8),dimension(nIons)::pHI_rates=0.
  real(kind=8) ::mu
  real(kind=8) ::mu_dp
  ...
  real(kind=8),dimension(1:ndim, 1:nGroups, 1:nvector):: Fp, dFpdt
  real(kind=8),dimension(1:ndim, 1:nvector):: p_gas
  real(kind=8),dimension(1:nvector)::nH=0., Zsolar=0.
