MODULE coolrates_module
  ...
  real(kind=8),dimension(nbinT) :: T_lookup = 0d0 ! Lookup temperature in log K
...
!-------------------------------------------------------------------------
  use mpi_mod
  use amr_commons,only:nstep_coarse
  implicit none
#ifndef WITHOUTMPI
  integer :: ierr
#endif
...
!-------------------------------------------------------------------------
  ...
  if(myid==0 .and. nstep_coarse .gt. 0) print*,'Coolrates table updated'
...
!-------------------------------------------------------------------------
  ...
  implicit none
  integer::iT
  real(kind=8)::aexp, T, T2, Ta, T5, lambda, f, hf, laHII, laHeII, laHeIII
...
!-------------------------------------------------------------------------
  ...
  !real(kind=8),dimension(nbinTK)::rates, ratesPrime
  real(kind=8),intent(in)::T
  logical,intent(in)::allow_negative
  real(kind=8),optional::retPrime
  real(kind=8)::inp_coolrates_table
  integer,save:: iT = 1
  real(kind=8),save:: facT, yy, yy2, yy3, fa, fb, fprimea, fprimeb, Tlast=-1
  real(kind=8),save:: alpha, beta, gamma
  logical,save::extrap
!-------------------------------------------------------------------------
...
!-------------------------------------------------------------------------
  ...
  real(kind=8),save::ci_HI,ci_HeI,ci_HeII
  real(kind=8),save::ci_HI_prime,ci_HeI_prime,ci_HeII_prime
  real(kind=8),save::cr_H2HI,cr_H2H2,cr_H2HI_prime,cr_H2H2_prime
  real(kind=8),save::ce_HI,ce_HeI,ce_HeII
  real(kind=8),save::ce_HI_prime,ce_HeI_prime,ce_HeII_prime
  real(kind=8),save::r_HII,r_HeII,r_HeIII
  real(kind=8),save::r_HII_prime,r_HeII_prime,r_HeIII_prime
  real(kind=8),save::bre, brefac, bre_prime, com, com_prime, die, die_prime
!-------------------------------------------------------------------------
