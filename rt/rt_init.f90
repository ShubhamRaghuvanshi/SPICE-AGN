...
!-------------------------------------------------------------------------
  ...
  if(rt .and. rt_AGN) then 
    call init_AGNSED_table    ! init stellar energy distribution properties 
  endif 
  !sinkagn_rt
#endif 
  ...
  !sinkagn_rt
#ifdef SINKAGN_RT
  if(rt .and. rt_AGN .and. myid==1) write(*,*) 'Radiation from AGN enabled'
#endif
  !sinkagn_rt
...
!-------------------------------------------------------------------------
  ...
  ! new lightspeed
  rt_c(ilevel) = dx/3.d0/dt * rt_courant_factor
  rt_c2(ilevel) = rt_c(ilevel)**2

  ! new ligtspeed in cgs
  call units(scale_l,scale_t,scale_d,scale_v,scale_nH,scale_T2)
  rt_c_cgs(ilevel) = rt_c(ilevel)*scale_v
  rt_c_fraction(ilevel) = rt_c_cgs(ilevel)/c_cgs

  call updateRTGroups_CoolConstants(ilevel)! These change as a consequence
...
!-------------------------------------------------------------------------
  namelist/rt_params/rt_star, rt_esc_frac, rt_flux_scheme, rt_smooth     &
       ...
       & ,cosmic_rays                                                    &
       & ,rt_tracemassbins, rt_tracemetalbins                            &
       & ,rt_traceagebins, showIonizationFraction                        &
...
101 continue                                   ! No harm if no rt namelist
  ...
  if(rt .and. rt_AGN) then 
    if(.not. sink) then 
      if(myid==1)write(*,*) "rt_AGN only works with sink enabled"
      nml_ok=.false.
    endif
  endif
  !sinkagn_rt 
  ...
  ! Shift lightspeed fractions from level 1 and up to levelmin and up
  ...
  ! Using level-variable speed of light?
  rt_vsla=.false.
  do ilevel=levelmin,nlevelmax-1
     if(rt_c_fraction(ilevel) .ne. rt_c_fraction(nlevelmax)) then
        rt_vsla=.true.
        if(myid==1)write(*,*) 'Using a level-variable speed of light, with f_c= '
  ...
  if(rt .or. neq_chem) then
    ...
    !sinkagn_rt
#ifdef SINKAGN_RT
    ! AGNRT: for now, only works with 3 standard groups+ions
    if (rt .and. rt_AGN) then
      if(nIonsUsed.eq.3 .and. nGroups.eq.3 .and. isHe .and. .not. isH2) then
        ! All is fine for AGN RT
        if(myid==1) write(*,*) 'Running with AGN radiation'
      else
        if(myid==1) then
          write(*,*) 'AGN radiation is only implemented for:'
          write(*,*) '- NIONS=3, while using NIONS=',nIonsUsed
          write(*,*) '- nGroups=3, while using nGroups=',nGroups
          write(*,*) '- no H2, but Helium ionizing radiation'
        end if
        nml_ok=.false.
      end if
    end if
...
!-------------------------------------------------------------------------
  namelist/rt_groups/group_csn, group_cse, group_egy, spec2group         &
      !sinkagn_rt
#ifdef SINKAGN_RT
      & , group_agnegy_frac, group_sed_update                           &
#endif 
      !sinkagn_rt
...
#ifdef SINKAGN_RT
  if (rt .and. rt_AGN) then
     group_csn_agn=0.0_dp; group_cse_agn=0.0_dp; group_egy_agn=0.0_dp
  end if
#endif 
  !sinkagn_rt
...
#ifdef SINKAGN_RT
     if(rt .and. rt_AGN) then
       group_csn_agn(igroup_HII, ixHII) = 3.181d-18
       group_cse_agn(igroup_HII, ixHII) = 2.990d-18
       group_egy_agn(igroup_HII) = 18.29
     end if
#endif  
     !sinkagn_rt
  endif
...
#ifdef SINKAGN_RT
    if (rt .and. rt_AGN) then
      group_csn_agn(igroup_HeII,ixHII) = 5.094d-19
      group_cse_agn(igroup_HeII,ixHII) = 4.663d-19
      group_csn_agn(igroup_HeII,ixHeII) = 4.235d-18
      group_cse_agn(igroup_HeII,ixHeII) = 3.993d-18
      group_egy_agn(igroup_HeII) = 35.63
    end if
#endif 
    !sinkagn_rt
  endif
...
#ifdef SINKAGN_RT
    if (rt .and. rt_AGN) then
      group_csn_agn(igroup_HeIII,ixHII) = 3.877d-20
      group_cse_agn(igroup_HeIII,ixHII) = 2.148d-20
      group_csn_agn(igroup_HeIII,ixHeII) = 6.349d-19
      group_cse_agn(igroup_HeIII,ixHeII) = 3.646d-19
      group_csn_agn(igroup_HeIII,ixHeIII) = 5.359d-19
      group_cse_agn(igroup_HeIII,ixHeIII) = 3.021d-19
      group_egy_agn(igroup_HeIII) = 128.3
    end if
#endif 
    !sinkagn_rt
  endif
...
!------------------------------------------------------------------------
  ...
  real(kind=8)::dx,dx_loc,scale
  real(dp),dimension(1:3)::skip_loc
  real(dp),dimension(1:twotondim,1:3)::xc
  real(dp),dimension(1:nvector,1:ndim),save::xx
  real(rtdp),dimension(1:nvector,1:nrtvartrace),save::uu
!------------------------------------------------------------------------
...
!------------------------------------------------------------------------
  ...
  integer ::nn
  real(dp)::dt
  real(kind=8)::dx,dx_cgs,dt_cgs
  real(rtdp),dimension(1:nvector,1:nrtvartrace)::uu
  real(dp),dimension(1:nvector,1:ndim)::x
  integer::i,k,group_ind,ilevel
  real(kind=8)::vol,r,xn,yn,zn,en
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v,nptot
