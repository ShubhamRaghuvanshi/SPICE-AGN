module rt_parameters
...
#endif
#ifdef NTRACEGROUPS
  integer,parameter::ntraceGroups=NTRACEGROUPS ! # of photon tracer groups (set in Makefile)
#else
  integer,parameter::ntraceGroups=0
#endif
  integer,parameter::nRTvar=nGroups*(1+ndim) ! # of RT variables (photon density and flux)
  integer,parameter::nRTvartrace=(nGroups*(1+ndim)) + (nGroups*ntraceGroups) ! # of RT variables (photon density and flux and tracers)
  ...
  real(dp),dimension(nGroups)::group_agnegy_frac=0               !  Fraction of agn energy
  real(dp),dimension(nGroups,nIons)::group_csn_agn=0, group_cse_agn=0 ! Cross sections (cm2)
  real(dp),dimension(nGroups)::group_egy_agn=0                   !  Avg photon energy (ev)
  logical,dimension(nGroups)::group_sed_update=.true.            !   Update SED properties
  !!!AGNRT
  ...
  real(rtdp),dimension(1:MAXBOUND,1:nrtvartrace)::rt_boundary_var
  ...
  ! Reionization statistics on the fly
  logical::showIonizationFraction=.false.
  ! Volume weighted ionization fraction
  real(dp),dimension(nIons)::ifrac_vweight_num, ifrac_vweight_den 
  ! Mass weighted ionization fraction
  real(dp),dimension(nIons)::ifrac_mweight_num, ifrac_mweight_den 
  ! Volume weighted HI photoionization rate
#ifdef NTRACEGROUPS
  real(dp),dimension(ntraceGroups+1)::prate_vweight_num 
#else
  real(dp)::prate_vweight_num
#endif
  real(dp)::prate_vweight_den 

  integer::minind=1 !Index of halo file
  logical::file_trigger=.false.

  real(dp),dimension(NTRACEGROUPS-1)::rt_tracemassbins=-1.0d0 !For mass photon tracers
  real(dp),dimension(NTRACEGROUPS-1)::rt_tracemetalbins=-1.0d0 !For metal photon tracers
  real(dp),dimension(NTRACEGROUPS-1)::rt_traceagebins=-1.0d0 !For stellar age photon tracers 


#ifdef NTRACEGROUPS
  real(dp),allocatable::haloprops(:,:)
  real(dp),allocatable::halopropz(:)
#endif
