...
!     tbl(:,:,2) = tbl(:,:,2) * Gyr2sec ! Convert from sec-1 to Myr-1
     ...
     ! SKIPPING THE INTERPOLATION HERE, TO SHOW DIRECTLY WHAT COMES OUT OF THE SEDS
...
!  SED_ages(1)=0.d0 ; SED_ages(2:)=rebAges ;    ! Must have zero initial age
  ...
  deallocate(ages)
  if(allocated(rebAges)) deallocate(rebAges) 
  deallocate(zs)
  deallocate(Ls)
...
#ifdef NTRACEGROUPS
  integer::dumpid
  real(dp)::adiff,adiffnew,nptot
  real(dp),dimension(1:ntraceGroups)::npfrac
...
SUBROUTINE get_tracer_group_from_halo(pos_star,groupindex,minind1)
  ...
  use rt_hydro_commons
  use rt_parameters
  real(dp),dimension(1:ndim)::pos_star,xdist
  integer::groupindex,minind1, hcount, i, mindcount, mindrcount
  real(dp)::distance,donrvir,mindist,minrdist
