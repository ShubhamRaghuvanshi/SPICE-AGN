...
subroutine output_frame()
 ...
 do proj_ind=1,LEN(trim(proj_axis)) 
  ...
  if(myid==1 .and. proj_ind==1)write(*,*)'Computing and dumping movie frame ', istep_str
...
#ifdef RT
  if(neq_chem) moviefiles(iIons) = trim(moviedir)//'xHII_'//trim(istep_str)//'.map'
  if(neq_chem) moviefiles(iIons+1) = trim(moviedir)//'xHeII_'//trim(istep_str)//'.map'
  if(neq_chem) moviefiles(iIons+2) = trim(moviedir)//'xHeIII_'//trim(istep_str)//'.map'
			        ...
			        imap = 1
                                do kk=1,NGROUPS
                                   if(rt_movie_vars(kk).eq.1) then
                                      uvar=rtuold(ind_cell(i),1+(kk-1)*(ndim+1))*rt_c(ilevel)
                                      if(method_frame(proj_ind).eq.'min')then
...
subroutine set_movie_vars()
  ...
  enddo
  if(ANY(movie_vars_txt=='xHII  ')) movie_vars(iChem)=1
  if(ANY(movie_vars_txt=='xHeII ')) movie_vars(iChem+1)=1
  if(ANY(movie_vars_txt=='xHeIII')) movie_vars(iChem+2)=1
#endif
