...
!-------------------------------------------------------------------------
  ...
  !Photon tracer stuff
  integer::hindex, hstart, hfin, tracegroup
...
!------------------------------------------------------------------------
        ...
        if(rt_is_outflow_bound) then
           do i=1,nexist
              if (rt_per_bnd(ind_exist(i))) then
                 uloc(ind_exist(i),i3,j3,k3,1:nrtvartrace) =  0.D0
              else              
                 do ivar=1,nrtvartrace
           ...
           do i=1,nbuffer
              if (rt_per_bnd(ind_nexist(i))) then
                 uloc(ind_nexist(i),i3,j3,k3,1:nrtvartrace) =  0.D0
              else
                 do ivar=1,nrtvartrace
  ...
  !----------------------------------------------------------------------
  do i = 1,nGroups
     !We dont want to send photon tracers into this function
     !it may be useful to put an intent(in) in rt_flux module so that this is not edited
     call cmp_rt_faces(uloc(1:nvector,iu1:iu2,ju1:ju2,ku1:ku2,1:nrtvar),flux,dx,dx,dx,dt,iGroups(i),ncache,ilevel)
  end do
           ...
           do i=1,ncache
            if (ntraceGroups.gt.0) then
                 if (ivar.eq.iGroups(tracegroup)) then                !This means we are updating the number density
                     ...
                     call update_tracers(cellphotdens,fluxdn,fluxup,tracedn,traceup,tracecell,0,ilevel) !Calculate the updated tracer fractions
              ...
              if (mod(ivar,ndim+1)==1) rtunew(ind_cell(i),ivar) = max(rtunew(ind_cell(i),ivar),smallNp)
           end do
        end do
     end do
     end do
...
SUBROUTINE update_tracers(Nphotcell,ndn,nup,fracdn,fracup,fraccell,signal,ilevel)
  use amr_commons
  use rt_parameters
  implicit none
