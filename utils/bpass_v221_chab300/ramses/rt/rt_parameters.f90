module rt_parameters
  ...
  real(dp),dimension(1:MAXLEVEL)::rt_c=0., rt_c2=0.! Const. in user units (set in init_rt)
  ...
  real(dp),dimension(1:MAXLEVEL)::rt_c_fraction=1.d0 ! Lightspeed fraction for RT        !
  ...
  real(dp),dimension(1:MAXBOUND,1:nrtvartrace)::rt_boundary_var
  ...
  integer,dimension(1:NGROUPS*NTRACEGROUPS)::rt_movie_vars_trace=0 ! For generating photon tracer movies
