...
subroutine rt_init_flow  
  use amr_commons
  use rt_hydro_commons, ONLY: nrtvar, rtuold, nrtvartrace
  implicit none
  ...
  if(ok_file)then
     ...
     do ivar=1,nrtvartrace
        ...
        if(ok_file3)then
           ...
           init_array=0d0
           ! If we are looking at a tracer variable, initialize to 1/ntracegroups
           if (ivar.gt.nrtvar) then
              init_array=1.0d0/ntraceGroups
           endif
        endif
...
!------------------------------------------------------------------------
  ...
  ! Set the tracer group fractions to 1/ntracegroups
  ! Note to harley, I will have to come back and fix this if we want to use a region
  do i=nrtvar+1,nrtvartrace
     uu(1:nn,i)=1.d0/ntracegroups
  enddo
