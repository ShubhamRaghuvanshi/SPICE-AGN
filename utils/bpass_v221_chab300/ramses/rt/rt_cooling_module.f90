    ...
    if(rt .and. nGroups .gt. 0) then
       ...
       if(is_kIR_T) then ! k_IR depends on T
          ...
          if(rt_T_rad) then  ! Use radiation temperature for kappa
             ...
             TR = max(T2_min_fix,(E_rad*rt_c_fraction(ilevel)/a_r)**0.25)
             dT2 = TR/mu ;   TK = TR
          endif
       ...
       dustAbs(:)  = kAbs_loc(:) *rho*Zsolar(icell)*rt_c_cgs(ilevel)
       dustSc(iIR) = kSc_loc(iIR)*rho*Zsolar(icell)*rt_c_cgs(ilevel)
    endif
