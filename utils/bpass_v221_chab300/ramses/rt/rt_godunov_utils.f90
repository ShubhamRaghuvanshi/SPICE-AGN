...
!###########################################################
!###########################################################                                     
SUBROUTINE get_rt_courant_coarse(dt,ilevel)
...
!-------------------------------------------------------------------------
  ...
  dt = rt_courant_factor*dx/3.d0/rt_c(ilevel)
...
subroutine rt_hydro_refine(ug,um,ud,ok,nn)
  ...
  ! dummy arguments
  integer nn
  real(dp)::ug(1:nvector,1:nrtvartrace)
  real(dp)::um(1:nvector,1:nrtvartrace)
  real(dp)::ud(1:nvector,1:nrtvartrace)
