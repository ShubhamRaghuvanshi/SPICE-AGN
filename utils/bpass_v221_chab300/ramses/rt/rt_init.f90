...
!-------------------------------------------------------------------------
  namelist/rt_params/rt_star, rt_esc_frac, rt_flux_scheme, rt_smooth     &
       ...
       & ,rt_isoPress, rt_isIRtrap, rt_tracemassbins, rt_tracemetalbins  &
       ...
       ! RT movies                                                       &
       & ,rt_movie_vars, rt_movie_vars_trace
...
101 continue                                   ! No harm if no rt namelist
  ...
  do i=nlevelmax,levelmin,-1 
     rt_c_fraction(i)=rt_c_fraction(i-levelmin+1)
  end do
  do i=1,levelmin-1          ! Just a dummy lightspeed for non-leaf levels
     rt_c_fraction(i)=1.0
  end do
  do i=nlevelmax,levelmin,-1 !     Set the light speed(s) according to f_c
     rt_c_cgs(i) = c_cgs * rt_c_fraction(i)
  end do

  ! Using level-variable speed of light?
  rt_vsla=.false.
  do i=levelmin,nlevelmax-1
     if(rt_c_fraction(i) .ne. rt_c_fraction(nlevelmax)) then
        rt_vsla=.true.
...
!------------------------------------------------------------------------
  ...
  real(dp),dimension(1:nvector,1:nrtvartrace),save::uu
!------------------------------------------------------------------------
...
!------------------------------------------------------------------------
  ...
  real(dp),dimension(1:nvector,1:nrtvartrace)::uu
  real(dp),dimension(1:nvector,1:ndim)::x
  integer::i,k,group_ind,ilevel
  real(dp)::vol,r,xn,yn,zn,en
  real(dp)::scale_nH,scale_T2,scale_l,scale_d,scale_t,scale_v
  real(dp)::scale_np,scale_fp,nptot
