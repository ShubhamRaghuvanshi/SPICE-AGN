...
!-------------------------------------------------------------------------
  ...
  integer::hindex,hstart,hfin,ntrace2,ntracegroups2
  ...
  allocate(rtuold(1:ncell,1:nrtvartrace))
  allocate(rtunew(1:ncell,1:nrtvartrace))
  rtuold=0.0d0 ; rtunew=0.0d0
  if (ntraceGroups.gt.0) then
     !here we assume that all photon tracers have an equal distribution to begin with
     !these update quickly once the first set of photons are emitted
     rtuold(1:ncell,nrtvar+1:nrtvartrace) = 1.d0/ntraceGroups
     rtunew(1:ncell,nrtvar+1:nrtvartrace) = 1.d0/ntraceGroups
  end if
  ...
  read(ilun)gamma2
  ntracegroups2=0
  if(nrtvar2.gt.nrtvartrace)then ! Not ok to drop RT variables 
     ntracegroups2 = (nrtvar2-nrtvartrace)/nGroups
     if(myid==1)then ! Not ok to drop RT variables 
        write(*,*)'File rt.tmp is not compatible (1)'
        write(*,*)'Found nrtvar  =',nrtvar2
        write(*,*)'Expected=',nrtvartrace
        write(*,*)'But assuming you know what you are doing....',ntracegroups2
        !call clean_stop
     endif
  end if
  do ilevel=1,nlevelmax2
     if (ilevel.lt.levelmin) rt_c(ilevel) = 1.d0
              ...
              do ivar=1,nGroups
                 ...
                 if (ntraceGroups.gt.0d0) then
                    ...
                    do idim=hstart,hfin
                       read(ilun)xx
                       do i=1,ncache
                          rtuold(ind_grid(i)+iskip,idim)=xx(i)
                       enddo
                    enddo
                 endif

                 if (ntraceGroups2.gt.0d0) then
                    ! Read photon tracers
                    do idim=1,ntracegroups2
                       read(ilun)xx
                    enddo
                 endif
