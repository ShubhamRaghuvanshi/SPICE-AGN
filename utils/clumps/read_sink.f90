program read_sink
  ...
  integer::ncpu,ncpu2,ndim, i, k, isink, icpu,nstar, nsink, nindsink, npart, npart2   
  integer:: ipos, ncpu_read
  real(KIND=8)::boxlen
  ...
  real(KIND=8),dimension(:,:),allocatable::sinkpos, sinkvel, jsink, bhspin
  real(KIND=8),dimension(:)  ,allocatable::m,sinkmass, sinkmass_prev, birth,dMsmbh,dMBH_coarse,dMEd_coarse
  ...
  write(*,*) "icpu : ", icpu
  call title(icpu,ncharcpu)
  nomfich=TRIM(repository)//'/sink_'//TRIM(nchar)//'.out'//TRIM(ncharcpu)
  open(unit=1,file=nomfich,status='old',form='unformatted')
  read(1)nsink
  read(1)nindsink
  allocate(id(1:nsink))
  allocate(m(1:nsink))
  allocate(sinkmass(1:nsink))
  allocate(sinkpos(1:nsink,1:ndim))
  allocate(jsink(1:nsink,1:ndim)) 
  allocate(bhspin(1:nsink,1:ndim))
  allocate(sinkvel(1:nsink,1:ndim))
  allocate(birth(1:nsink))
  allocate(dMsmbh(1:nsink)) 
  allocate(dMBH_coarse(1:nsink)) 
  allocate(dMEd_coarse(1:nsink)) 
  ...
  write(346,'(I10,12(A1,ES20.10))')id(isink),',',sinkmass(isink),&
          ...
          ',',t-birth(isink),',',dMsmbh(isink), ',' ,bhspin(isink,0), ',', bhspin(isink,1), ',', bhspin(isink,2) 
  end do   
  close(346)
  ...
  deallocate(birth)
  deallocate(dMsmbh) 
  deallocate(dMBH_coarse) 
  deallocate(dMEd_coarse) 
