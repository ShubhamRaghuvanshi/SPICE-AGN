subroutine read_hydro_params(nml_ok)
  ...
  namelist/feedback_params/eta_sn,eta_ssn,yield,rbubble,f_ek,ndebris &
       ...
       & ,eta_rap,tau_IR,mstarparticle,KMT09,vmaxFB,ysc_stats &
...
! MHD sets enums for riemann solvers, hydro just does a string compare
  ...
  !if(TRIM(feedback).eq.'mechanical')   f_w=-1       ! prevent thermal or kinetic feedback 
  !if(TRIM(feedback).eq.'agertz') use_initial_mass=.true. ! for continuous SN2
  ...
  imetal=inener+nener
  idelay=imetal
  if(metal)idelay=imetal+2 !Oscar: two metal variables
  ivirial1=idelay
  ivirial2=idelay
