...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  real(kind=8)::work,Npc,fred,Npnew, kIR, E_rad, TR
  real(kind=8),dimension(1:ndim)::Fpnew
  real(kind=8),dimension(nIons, 1:nvector),save:: xion
  real(kind=8),dimension(nGroups, 1:nvector),save:: Np, Np_boost=0d0, dNpdt=0d0
  real(kind=8),dimension(ndim, nGroups, 1:nvector),save:: Fp, Fp_boost=0, dFpdt=0
  real(kind=8),dimension(ndim, 1:nvector),save:: p_gas, u_gas
  real(kind=8)::f_trap, NIRtot, EIR_trapped, unit_tau, tau, Np2Ep
  real(kind=8)::aexp_loc, f_dust, xHII
  real(kind=8),dimension(nDim, nDim):: tEdd ! Eddington tensor
  real(kind=8),dimension(nDim):: flux 
#endif
#if NENER>0
  integer::irad
#endif
  ...
  do ind=1,twotondim
     ...
     ! Compute rho
     do i=1,nleaf
        uold(ind_leaf(i),1) = max(uold(ind_leaf(i),1),smallr) !Oscar, precation
     ...
     if(metal)then
        do i=1,nleaf
           uold(ind_leaf(i),imetal)=max(uold(ind_leaf(i),imetal),0.0)  !avoid negative Z
...
!           Zsolar(i)=uold(ind_leaf(i),imetal)/nH(i)/0.02
           Zsolar(i)=(2.09d0*uold(ind_leaf(i),imetal+1) + &
                & 1.06d0*uold(ind_leaf(i),imetal))/nH(i)/0.02d0 !Asplund solar mix
        end do
     else
        do i=1,nleaf
           Zsolar(i)=z_ave
     ...
     if(cooling_cr)then
        do i=1,nleaf
           ! mue~1
           tcr_cool=mH/(7.51d-16*nH(i)*scale_d)
           tcr_heat=mH/(2.63d-16*nH(i)*scale_d)
           uold(ind_leaf(i),ndim+2) = T2(i) + T2min(i) + ekk(i) + emag(i) &
                & + err(i)*(exp(-dtcool/tcr_cool)+tcr_cool/tcr_heat*(1d0-exp(-dtcool/tcr_cool)))
           uold(ind_leaf(i),inener) = err(i)*exp(-dtcool/tcr_cool) 
           Etotcool=Etotcool+err(i)*(1.0d0-exp(-dtcool/tcr_cool))*vol_loc
        end do
     endif
#endif
...
!------------------------------------------------------------------------
  ...
  implicit none
  real(kind=8)::Npc
  real(kind=8),dimension(1:ndim)::Fp ,u
  real(kind=8),dimension(1:ndim,1:ndim)::T_Edd 
