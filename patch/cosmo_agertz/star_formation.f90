...
subroutine star_formation(ilevel)
  ...
  logical ::ok_free
  real(dp)::d,x,y,z,u,v,w,e,tg,zg,zg2
  real(dp)::mstar,mstar1,dstar,tstar,nISM,nCOM,phi_t,phi_x,theta,sigs,scrit,b_turb,zeta
  ...
  ! Initial star particle mass -- Oscar addition
  mstar=(mstarparticle*2.0d33)/scale_d/scale_l/scale_l/scale_l  !mstar in internal units
  dstar=mstar/vol_loc
     ...
     do ind=1,twotondim
        ...
        if(sf_virial)then
           do i=1,ngrid
              ...
              ! --------- Density and T criterion       Oscar: do here to avoid extra computation
              if(uold(ind_cell(i),1)<=d0) ok(i)=.false.   
              ...
              if (ok(i)) then
                 ...
                 if(sf_tdiss.gt.0d0) then
                    ...
                    if(sf_compressive)then
                       ...
                       uold(ind_cell(i),ivirial1) = sigma2   !Oscar: Remove to avoid another variable
                    endif
                 endif
                 ! Density criterion   !Oscar: superflouos, but keep anyway
                 if(d<=d0) ok(i)=.false.   
                 if(ok(i)) then
                    SELECT CASE (sf_model)
                       ...
                       CASE (4)
                          ! Feedback efficiency
                          t_dyn     = dx_loc/(2.0*sqrt(sigma2)) !+cs2))! Oscar check
           ...
           do i=1,ngrid
              d=uold(ind_cell(i),1)
              if(d<=d0)ok(i)=.false.   !always active, Oscar
        ...
        do i=1,ngrid
           ...
           if(ok(i))then
              ...
              if(KMT09)then  !Calculate f_H2, which enters mcell below.
                 Zscale=(2.09d0*uold(ind_cell(i),imetal+1)+1.06d0*uold(ind_cell(i),imetal))/0.02d0 
              ...
              mgas=dtnew(ilevel)*(sfr_ff(i)/tstar)*mcell*f_h2 !accounts for self-shielded gas
              ! Poisson mean
              ...
              if (mgas > 0.9*mcell) then
                 nstar_corrected=int(0.9*mcell/mstar1)
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           if(metal)zg=uold(ind_cell_new(i),imetal)     ! Fe
           if(metal)zg2=uold(ind_cell_new(i),imetal+1)  ! O
           ...
           vp(ind_part(i),2)=v
           vp(ind_part(i),3)=w
           if(metal)zp(ind_part(i))=zg    ! Initial Fe
           if(metal)zp2(ind_part(i))=zg2  ! Initial O
