                 ...
                 if(ok(i)) then
                    SELECT CASE (sf_model)
                       ...
                       CASE (1)
                          ...
                          phi_x     = 1.12
                          ! The prefered value for eps_star_loc = 1.0,
                          ! which represents the theoretical maximum efficiency.
                          sigs      = log(1.0+(b_turb**2)*(M2))
                          scrit     = log(((pi**2)/5)*(phi_x**2)*alpha0*(M2))
                          sfr_ff(i) = (eps_star_loc/2.0)*exp(3.0/8.0*sigs)*(2.0-erfc((sigs-scrit)/sqrt(2.0*sigs)))
                       ...
                       CASE (2)
                          ...
                          sfr_ff(i) = (eps_star_loc/(2.0*phi_t))*exp(3.0/8.0*sigs)*(2.0-erfc((sigs-scrit)/sqrt(2.0*sigs)))
  ...
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        do i=1,nnew
           ...
           mp0(ind_part(i))=n*mstar     ! Initial mass
