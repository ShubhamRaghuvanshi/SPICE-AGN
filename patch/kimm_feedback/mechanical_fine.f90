...
subroutine mech_fine(ind_grid,ind_pos_cell,np,ilevel,nSN,mSN,pSN,mZSN,nphSN,mchSN)
  ...
  integer,dimension(1:nvector)::icellv
  integer::ind_cell,ncell,ii
  ...
  real(dp)::ul,vl,wl,ur,vr,wr
  real(dp)::pvar(1:nvarMHD)
  real(dp)::vturb,vth,Mach,sig_s2,dratio
...
!        stop
     ...
     if(loading_type.eq.1)then
        ...
        ncell = 1
        icellv(1)=icell
        call getnbor(icellv,ind_nbor,ncell,ilevel)
        u0 = uold(icell        ,2) 
        v0 = uold(icell        ,3) 
        w0 = uold(icell        ,4) 
