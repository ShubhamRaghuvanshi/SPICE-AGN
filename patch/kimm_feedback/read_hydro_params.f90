...
subroutine read_hydro_params(nml_ok)
  ...
  ! Kimm parameters
  namelist/kimm_params/ &
       & T2thres_SF, mechanical_feedback, sn2_real_delay, fstar_min, M_SNII       &
       & ,log_mfb, log_mfb_mega, use_initial_mass, mechanical_geen                 &
       & ,star_imf, n_dc, n_gmc, nsn2mass, star_maker, SN_dT2_min         &
       & ,A_SN, expN_SN, E_SNII, SF_kick_kms, sf_lam
...
#endif
  read(1,NML=kimm_params,END=110)
  if(myid==1) write(*,*) 'You must have kimm_params in your namelist' 
110 continue
  rewind(1)
...
! MHD sets enums for riemann solvers, hydro just does a string compare
  ...
  if(mechanical_feedback)   f_w=-1                  ! prevent thermal or kinetic feedback 
