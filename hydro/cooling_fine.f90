...
subroutine coolfine1(ind_grid,ngrid,ilevel)
  ...
  do ind=1,twotondim
     ...
     ! Compute rho
     do i=1,nleaf
        uold(ind_leaf(i),1) = max(uold(ind_leaf(i),1),smallr) !Oscar, precaution
     ...
     if(metal)then
        do i=1,nleaf
           ...
           if(is_oxygen) then
              ...
              !Asplund solar mix
              Zsolar(i)=Zsolar(i)+2.09d0*uold(ind_leaf(i),imetal+1)/nH(i)/0.02d0 
           endif
        end do
     else
        do i=1,nleaf
     ...
     if(rt_isIRtrap) then
        ...
        do i=1,nleaf
           ...
           if (Zsolar(i) .lt. 1.05 * z_ave) then
               f_dust = 1d-10
           endif 
