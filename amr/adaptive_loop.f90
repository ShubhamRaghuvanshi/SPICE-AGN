subroutine adaptive_loop
  ...
  if(trim(feedback_model).eq.'mechanical') call init_mechanical
  ...
  !shubh
  do isink=1,nsink
    !write(*,*),'mass ratio :', isink,myid,Efeed_all(isink)/Efeed_new(isink)
    dMsmbhoverdt_out(isink) = 0d0
    dMBHoverdt_out(isink)   = 0d0
    dMEddoverdt_out(isink)  = 0d0
  enddo 
#endif 
...
#ifdef SINKTEST
          !do isink=1,nsink
          !  Efeed(isink) = 0.15*0.1*dMsmbhoverdt_out(isink)
          !enddo 
          !if(myid==1) then 
          !write(*,*) "acc_rates :", dMsmbhoverdt_out(1),dMBHoverdt_out(1),dMEddoverdt_out(1) 
          !endif 

          if(myid==1)then
            call write_sink_props   
          endif  
    ...
    if(mod(nstep_coarse,ncontrol)==0)then
      ...
      if(myid==1)then
        if (tot_pt==0) muspt=0.0 ! dont count first timestep
      ...
      if(walltime_hrs.gt.0d0) then
        wallsec = walltime_hrs*3600.0    ! Convert from hours to seconds
        dumpsec = minutes_dump*60.0       ! Convert minutes before end to seconds
