...
! Precision for radiative transfer
  ...
  logical::and_then_stop=.false.                  ! to call clean_end if true
  ...
  real(dp)::omega_b=0.0486                       ! Omega Baryon
...
!  real(dp)::n_sink = -1                           ! Sink particle density threshold in H/cc
  ...
  real(dp)::eps_star_loc=0.5d0                    ! Local star formation efficiency: unresolved proto-stellar feedback
  ...
  real(dp)::del_star=0                          ! Minimum overdensity to define ISM
...
!  real(dp)::ind_rsink=4                           ! Number of cells defining the radius of the sphere where AGN feedback is active
  ...
  logical ::SFdiagnostics=.false.                 ! Make log files for SF 
  ...
  integer::i_mv_fp=-1,    i_mv_pmag=-1,       i_mv_nener=-1
  ...
  character(len=10)::feedback_model=''     ! 'mechanical', 'agertz', 'mom2', 'kinetic'
  ...
  character(len=10)::star_maker='density'         ! density, federrath, FK2 or none
...
#ifdef SINKTEST 
  !run_params
  logical::read_ic_sink=.false. !Read init_sink if available
  logical::sinkprops=.false.  ! Write sink properties at each coarse step
  !run_params

  !movie_params
  integer::follow_sink=0   !Id of the sink to follow throughout the simulation
  !movie_params
  ...
  real(dp)::ind_rsink=4       ! Number of cells defining the radius of the sphere where AGN feedback is active
  real(dp)::rAGN   =1.2D0     ! AGN superbubble radius in kpc
  ...
  logical ::maximum_accretion=.false.  !Accrete all gas within the BH's accretion region. 
  ...
  logical::output_finestep_sink=.false.     !write finestep sink information
  logical ::accrete_momentum=.false.           ! Whether the BH accretes momentum or not
