recursive subroutine amr_step(ilevel,icount)
  ...
  if(ilevel==levelmin)then
    ...
    if(mod(nstep_coarse,foutput)==0.or.aexp>=aout(iout).or.t>=tout(iout).or.output_now_all.EQV..true.)then
      ...
      if (and_then_stop.EQV..true.) call clean_finish
  ...
  if(ilevel==levelmin)then
    ...
    if(hydro.and.star.and.eta_sn>0.and.f_w>0 .and. trim(feedback_model).eq.'kinetic') then 
    ...
    if(sink) then
      call timer('sinks','start')
      if (sink_AGN .and. (.not. finestep_AGN)) then  
        call AGN_feedback
      endif   
      !-----------------------------------------------------
  ...
  if(poisson)then
    ...
    if(hydro)then
      ...
      if(sink)then
        ...
        !if(myid==1)write(*,*), "done with grow bondi for level :", ilevel
      endif
#endif 
      !sinktest
  ...
  !******************** Make it in two seperate functions for AGN and non AGN case
  !sinktest 
