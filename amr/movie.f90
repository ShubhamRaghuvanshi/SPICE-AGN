...
#if NENER>0
                                ! NENER (for CR) energy map case
                                else if(movie_vars(kk).eq.i_mv_nener)then
                                   ok_frame=.true.
                                   uvar = uold(ind_cell(i),inener)
#endif
                                endif
...
subroutine set_movie_vars()
  ...
  do kk=1,n_movie_vars
     ...
     else if (movie_vars_txt(kk) .eq. 'Pcr') then
        if(i_mv_nener .eq. -1) i_mv_nener = kk
        movie_vars(kk) = i_mv_nener
