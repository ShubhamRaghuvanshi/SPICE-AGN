...
#endif
#ifdef RT
  real(dp),dimension(1:nvector,0:twondim  ,1:nrtvartrace),save::urt1
  real(dp),dimension(1:nvector,1:twotondim,1:nrtvartrace),save::urt2
#endif  
...
subroutine kill_grid(ind_cell,ilevel,nn,ibound,boundary_region)
  ...
  if(upload_equilibrium_x) then
     ...
     do i=1,nn                                                        
        call calc_equilibrium_xion(uold(ind_cell(i),1:nvar) &
             , rtuold(ind_cell(i),1:nrtvartrace), xion, ilevel)    
  ...
  do ind=1,twotondim
     ...
     if(rt)then
        do ivar=1,nrtvartrace
           do i=1,nn
              rtuold(ind_cell_son(i),ivar)=0.0
              rtunew(ind_cell_son(i),ivar)=0.0
           end do
        end do
     end if
#endif
